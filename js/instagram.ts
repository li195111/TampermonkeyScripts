...
(function () {
  ...
  function parse_stories() {
    ...
    let account_e: HTMLAnchorElement | null = document.querySelector('div.x5yr21d div.x16adzlw div.x6s0dn4 div.x6s0dn4');
    var account_str: string = '';
    if (account_e != null && account_e.textContent != null) {
    ...
    if (curr_story != null) {
      ...
      var img: HTMLImageElement | null = curr_story.querySelector('img.x19kjcj4');
      var src: string;
      if (img != null) {
        src = img.src;
  ...
  function mainPageArticles() {
    ...
    let articleDetectorID = setInterval(() => {
      articles = Array.from(document.querySelectorAll('article._aatb'));
      console.log(`Detect : ${articles.length} Articles`);
